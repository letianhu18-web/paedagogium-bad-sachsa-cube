...
export default function LeadershipPage() {
  return (
    <>
      <PageIntro eyebrow="參 · SCHULLEITUNG" title="校长和管理人员" subtitle="Schulleitung, Koordination und Verwaltung" description="人员、职务与工作联系方式均来自学校官网 Kontakt 页面。没有使用私人地址、私人电话或非官方社交信息。" accent="#75a97f" />
      <section className="mx-auto mt-12 max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="glass-soft mb-8 flex flex-col gap-3 rounded-2xl p-5 text-sm text-emerald-50/75 sm:flex-row sm:items-center sm:justify-between">
          <span className="inline-flex items-center gap-2"><ShieldCheck size={17} className="text-amber-200" /> 仅展示官网公开的校内工作信息</span>
          <span className="inline-flex items-center gap-2 text-xs text-emerald-100/45"><CalendarCheck size={14} /> 最后核实：{leadershipVerifiedAt}</span>
        </div>
          ...
          {leadership.map((leader, index) => (
            <article key={leader.name} className="glass group flex min-h-[330px] flex-col rounded-[2rem] p-6 transition hover:-translate-y-1">
              <div className="flex items-start justify-between gap-4">
                <span className="grid size-14 place-items-center rounded-2xl bg-gradient-to-br from-emerald-200/18 to-amber-200/5 text-sm font-semibold text-amber-100 ring-1 ring-emerald-200/20">{teacherInitials(leader.name)}</span>
              ...
              <p className="mt-2 text-sm font-medium text-amber-200/80">{leader.roleDe}<span className="mx-1.5 text-emerald-100/25">｜</span>{leader.roleZh}</p>
        ...
        <div className="glass-soft mt-8 rounded-3xl p-6">
