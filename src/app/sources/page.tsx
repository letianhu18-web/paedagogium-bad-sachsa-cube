...
export default function SourcesPage() {
  return (
    <>
      <PageIntro eyebrow="玉册 · QUELLEN" title="资料来源" subtitle="Öffentlich zugängliche Informationen" description="网站的重要事实均附有可访问的公开来源。优先使用学校官网，并记录获取与最后核实日期。" accent="#6b9f7e" />
      <section className="mx-auto mt-12 max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-3">
          {sources.map((source, index) => (
            <article key={source.url} className="glass-soft grid gap-4 rounded-3xl p-5 sm:grid-cols-[42px_1fr_auto] sm:items-start sm:p-6">
              <span className="grid size-9 place-items-center rounded-xl bg-emerald-300/10 font-mono text-xs text-amber-100 ring-1 ring-emerald-200/16">{String(index + 1).padStart(2, "0")}</span>
              <div>
              ...
              <a href={source.url} target="_blank" rel="noreferrer" className="focus-ring jade-button-secondary inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs">打开 <ExternalLink size={12} /></a>
            </article>
          ))}
        </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-3xl border border-amber-200/12 bg-amber-200/[0.035] p-6">
            <SearchCheck className="text-amber-200/75" size={20} />
