...
export default async function TeacherDetailPage({ params }: TeacherPageProps) {
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 pb-6 pt-14 sm:px-6 sm:pt-20 lg:px-8">
      <Link href="/teachers" className="focus-ring jade-button-secondary inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm"><ArrowLeft size={15} /> 返回教师列表</Link>
      <div className="glass mt-8 overflow-hidden rounded-[2rem]">
        <div className="relative border-b border-emerald-100/10 p-6 sm:p-10">
          <div className="absolute -right-20 -top-20 size-64 rounded-full bg-emerald-500/12 blur-[80px]" />
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center">
            {teacher.photo ? (
              <span className="relative block size-28 shrink-0 overflow-hidden rounded-[2rem] ring-1 ring-emerald-100/16 sm:size-32">
            ...
            <div>
              ...
              <h1 className="jade-title mt-3 text-3xl font-semibold tracking-tight sm:text-5xl">{teacher.name}</h1>
        ...
        <div className="grid gap-8 p-6 sm:p-10 lg:grid-cols-[1.1fr_.9fr]">
          <div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {teacher.subjects.map((subject) => (
                <div key={subject.de} className="glass-soft rounded-2xl p-4">
          ...
          <aside className="jade-control rounded-3xl p-5 sm:p-6">
            ...
            <div className="my-5 h-px bg-gradient-to-r from-transparent via-emerald-100/16 to-transparent" />
      ...
      <div className="glass-soft mt-5 rounded-2xl p-4 sm:flex sm:items-center sm:justify-between sm:gap-4">
        <div className="flex min-w-0 items-start gap-3">
          <Link2 className="mt-0.5 shrink-0 text-amber-200/70" size={16} />
          <div className="min-w-0">
            <p className="text-xs text-slate-500">个人页面网址 · 本地运行时</p>
            <p className="mt-1 truncate font-mono text-xs text-emerald-100/65">http://localhost:3000{teacher.profilePath}</p>
          </div>
        </div>
        <Link href={teacher.profilePath} className="focus-ring jade-button-secondary mt-3 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs sm:mt-0">
